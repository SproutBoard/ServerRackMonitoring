...
{
    _BounceList[ _switchCount ] = new Bounce( pin, 5);
...
void ReedSwitches::PrintSerial( )
{
    for ( int i = 0; i < _switchCount; i++ )
    {
        if ( _switchList[ i ] != DIGITAL_PIN_NONE )
        {
            int value = _BounceList[i] -> read();
...
void ReedSwitches::PrintWeb(EthernetClient client )
{
    for ( int i = 0; i < _switchCount; i++ )
    {
        if ( _switchList[ i ] != DIGITAL_PIN_NONE )
        {
            int value = _BounceList[i] -> read();
            client.print("Reed Switch on Pin ");
            client.print(i);
            client.print(": ");
            client.print(value == HIGH? "High" : "Low");
            client.println("<br/>");
        }
    }
}
bool ReedSwitches::Poll()
{
    ...
    for ( int i = 0; i < _switchCount; i++ )
    {
        _BounceList[i] -> update();
        if (_BounceList[i] -> read() == HIGH )
        {
            result = true;
